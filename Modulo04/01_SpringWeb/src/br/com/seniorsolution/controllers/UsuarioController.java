		...
		try {
			ApplicationContext ctx = 
				...
				if (dao.validarUsuario(
					...
					Usuario userBuscado = dao.validarUsuario(usuario.getNome(), Utilitarios.gerarMD5(usuario.getSenha()));
					...
					throw new Exception();
					//model.addObject("mensagem", "Usuário ou senha inválidos");					
				}
				
		} catch (Exception e) {
			model.addObject("mensagem", "Usuario ou senha inválidos");
	...
	public ModelAndView incluir(Usuario usuario) {
		...
		try {
			ApplicationContext ctx = 
				...
				model.addObject("mensagem", "Usuário " 
						+ usuario.getNome() + " incluído.");
